...
void solve() {
    ...
    ll curr = 0,cnt = 0,i = 0, j = 0;
    int size = 0;
    while(j < n){
        if(j < n && curr + a[j] <= s){
            curr += a[j++];
            cnt+= (j-i); //this way 
            //ex when we add 2 6 4 add(3) to our 2 6 4 segment
            //we consider all the posibbility here only
            // like  2643 643 43 3
           // which is equal to the size of segment itself
        }else{
            curr -= a[i++];
        }
    }
