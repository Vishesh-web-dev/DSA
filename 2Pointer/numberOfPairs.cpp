...
void solve() {
    ...
    vll sa(a.begin(),a.end());
    srt(sa);
    ll cnt = 0;
    for(int i = 0 ; i < n ; i++){
        ll low = l-a[i];
        ll high = r-a[i];
        auto itr1 = lower_bound(sa.begin()+i+1,sa.end(),low+1) - sa.begin();
        auto itr2 = upper_bound(sa.begin()+i+1,sa.end(),high-1) - sa.begin() -1;
        if((itr1 >= n) || (itr1 > itr2) || (itr2 >= n)){
            continue;
        }
        cnt += itr2-itr1+1;
    }
